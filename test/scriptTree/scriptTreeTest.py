...
class TestCase(unittest.TestCase):
    ...
    def testScriptTree_Example(self):
        ...
        for test in xrange(self.testNo):
            command = "scriptTreeTest_Wrapper.py --jobTree %s --logLevel=DEBUG --retryCount=10" % self.jobTreeDir
            system(command)
    ...
    def testScriptTree_Example2(self):
        ...
        """
        for test in xrange(self.testNo):
            command = "scriptTreeTest_Wrapper2.py --jobTree %s --logLevel=DEBUG --retryCount=10" % self.jobTreeDir
            system(command)
            runJobTreeStatusAndFailIfNotComplete(self.jobTreeDir)
