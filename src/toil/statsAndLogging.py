...
class StatsAndLogging:
    ...
    @classmethod
    def formatLogStream(
        cls, stream: Union[IO[str], IO[bytes]], job_name: Optional[str] = None
    ) -> str:
        """
    ...
    def statsAndLoggingAggregator(
        ...
        text = json.dumps(
            dict(
                ...
                total_clock=str(get_total_cpu_time() - startClock),
            ),
            ensure_ascii=True,
        )
...
def add_logging_options(parser: ArgumentParser) -> None:
    ...
    for level in levels:
        group.add_argument(
            ...
            action="store_const",
            const=level,
            help=f"Turn on loglevel {level}.  Default: {default_loglevel}.",
        )
