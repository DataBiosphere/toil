...
defaultRequirements = dict(
    memory=int(100e6), cores=1, disk=1000, preemptable=preemptable, accelerators=[]
)
...
class BatchSystemPluginTest(ToilTest):
    ...
    def testAddBatchSystemFactory(self):
        ...
        assert (
            "testBatchSystem",
            test_batch_system_factory,
        ) in BATCH_SYSTEM_FACTORY_REGISTRY.items()
    ...
    class AbstractBatchSystemTest(ToilTest, metaclass=ABCMeta):
        ...
        def test_set_job_env(self):
            ...
            job6 = self.batchSystem.issueBatchJob(
                job_desc_6, job_environment={"FOO": "bar"}
            )
...
class ParasolBatchSystemTest(hidden.AbstractBatchSystemTest, ParasolTestSupport):
    ...
    def createBatchSystem(self) -> AbstractBatchSystem:
        ...
        return ParasolBatchSystem(
            config=self.config, maxCores=numCores, maxMemory=memory, maxDisk=1001
        )
