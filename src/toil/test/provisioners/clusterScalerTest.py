...
c4_8xlarge_preemptable = Shape(
    wallTime=3600, memory=h2b("60G"), cores=36, disk=h2b("100G"), preemptable=True
)
# simplified c4.8xlarge (non-preemptable)
c4_8xlarge = Shape(
    wallTime=3600, memory=h2b("60G"), cores=36, disk=h2b("100G"), preemptable=False
)
# simplified r3.8xlarge (non-preemptable)
r3_8xlarge = Shape(
    wallTime=3600, memory=h2b("260G"), cores=32, disk=h2b("600G"), preemptable=False
)
# simplified r5.2xlarge (non-preemptable)
r5_2xlarge = Shape(
    wallTime=3600, memory=h2b("64Gi"), cores=8, disk=h2b("50G"), preemptable=False
)
# simplified r5.4xlarge (non-preemptable)
r5_4xlarge = Shape(
    wallTime=3600, memory=h2b("128Gi"), cores=16, disk=h2b("50G"), preemptable=False
)
# simplified t2.micro (non-preemptable)
t2_micro = Shape(
    wallTime=3600, memory=h2b("1G"), cores=1, disk=h2b("8G"), preemptable=False
)
...
class BinPackingTest(ToilTest):
    ...
    def testPackingOneShape(self):
        ...
        self.assertEqual(
            [x.shapes() for x in self.bpf.nodeReservations[c4_8xlarge_preemptable]],
            [
                [
                    Shape(
                        wallTime=1000,
    ...
    def testPathologicalCase(self):
        ...
        self.assertNotEqual(
            self.bpf.getRequiredNodes(), {r3_8xlarge: 1, c4_8xlarge_preemptable: 0}
        )
...
class ClusterScalerTest(ToilTest):
    ...
    def _check_job_estimate(
        ...
        jobs = [
            Shape(
                wallTime=3600, cores=cores, memory=memory, disk=disk, preemptable=True
            )
        ]
...
class ScalerThreadTest(ToilTest):
    def _testClusterScaling(self, config, numJobs, numPreemptableJobs, jobShape):
        ...
        try:
            ...
            list(
                map(
                    lambda x: mock.addJob(jobShape=jobShape, preemptable=True),
            ...
            while (
                mock.getNumberOfJobsIssued(preemptable=False) > 0
    ...
    def testClusterScalingWithPreemptableJobs(self):
        ...
        self._testClusterScaling(
            config, numJobs=100, numPreemptableJobs=100, jobShape=job_shape
        )
    ...
    def getNumberOfJobsIssued(self, preemptable=None):
        if preemptable is not None:
            jobList = [
                ...
                if self.jobBatchSystemIDToIssuedJob[job].preemptable == preemptable
            ]
            return len(jobList)
        else:
