...
def runQC(job, cwl_file, cwl_filename, yml_file, yml_filename, outputs_dir, output_num):
    ...
    cwl = job.fileStore.readGlobalFile(
        cwl_file, userPath=os.path.join(tempDir, cwl_filename)
    )
    yml = job.fileStore.readGlobalFile(
        yml_file, userPath=os.path.join(tempDir, yml_filename)
    )
...
if __name__ == "__main__":
    ...
    with Toil(options) as toil:
        ...
        inputs_dir = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "cwlExampleFiles"
        )
        ...
        cwl_file = toil.importFile(
            "file://" + os.path.abspath(os.path.join(inputs_dir, cwl_filename))
        )
        ...
        for yml in yml_files:
            i = i + 1
            yml_file = toil.importFile(
                "file://" + os.path.abspath(os.path.join(inputs_dir, yml))
            )
            yml_filename = yml
            job = Job.wrapJobFn(
                runQC,
