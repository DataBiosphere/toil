...
class WdlStandardLibraryFunctionsTest(ToilTest):
    ...
    def testFn_Sub(self):
        ...
        self.assertEqual(
            "I love chocolate when it's late", sub(chocolike, "like", "love")
        )
        self.assertEqual(
            "I like chocoearly when it's early", sub(chocolike, "late", "early")
        )
        self.assertEqual(
            "I like chocolate when it's early", sub(chocolike, "late$", "early")
        )
    ...
    def testFn_WriteJson(self):
        ...
        self._check_output(
            path,
            '{"str":"some string","num":3.14,"bool":true,"null":null,"arr":["test"]}',
        )
        ...
        self._check_output(
            path,
            '{"left":1,"right":{"left":2,"right":{"left":3,"right":{"left":4,"right":5}}}}',
        )
...
class WdlWorkflowsTest(ToilTest):
    ...
    def check_function(
        ...
        """
        wdl_files = [
            os.path.abspath(f"{self.test_path}/{function_name}_{case}.wdl")
            for case in cases
        ]
        json_file = os.path.abspath(
            f"{self.test_path}/{json_file_name or function_name}.json"
        )
        for wdl_file in wdl_files:
            with self.subTest(f"Testing: {wdl_file} {json_file}"):
                ...
                if expected_exception is not None:
                    ...
                    self.fail(
                        "Invalid test. Either `expected_result` or `expected_exception` must be set."
                    )
...
class WdlLanguageSpecWorkflowsTest(WdlWorkflowsTest):
    ...
    def test_type_pair(self):
        ...
        expected_result = (
            '[23,"twenty-three","a.bai",{"left":23,"right":"twenty-three"}]'
        )
        self.check_function(
            "type_pair", cases=["basic"], expected_result=expected_result
        )
        ...
        self.check_function(
            "type_pair", cases=["with_files"], expected_result=expected_result
        )
    ...
    def test_v1_declaration(self):
        ...
        self.check_function(
            "v1_spec", cases=["declaration"], expected_result=expected_result
        )
...
class WdlStandardLibraryWorkflowsTest(WdlWorkflowsTest):
    ...
    def test_sub(self):
        ...
        self.check_function(
            "sub", cases=["as_input"], expected_result="src/toil/test/wdl/test.csv"
        )
        ...
        self.check_function(
            "sub", cases=["as_input_with_file"], expected_result="test.csv"
        )
    ...
    def test_size(self):
        ...
        self.check_function(
            "size", cases=["as_output"], expected_result="22.0 0.022 0.021484375"
        )

    def test_ceil(self):
        self.check_function(
            "ceil", cases=["as_input", "as_command"], expected_result="12"
        )

    def test_floor(self):
        self.check_function(
            "floor", cases=["as_input", "as_command"], expected_result="11"
        )

    def test_round(self):
        self.check_function(
            "round", cases=["as_input", "as_command"], expected_result="11"
        )

    def test_stdout(self):
        self.check_function(
            "stdout", cases=["as_output"], expected_result="A Whale of a Tale."
        )
        self.check_function(
            "stderr",
            cases=["as_output"],
            expected_result="a journey straight to stderr",
        )
    ...
    def test_read(self):
        ...
        self.check_function(
            "read_tsv", cases=["as_output"], expected_result="1\t2\t3\n4\t5\t6\n7\t8\t9"
        )
        ...
        self.check_function(
            "read_string", cases=["as_command"], expected_result="A Whale of a Tale."
        )
        self.check_function(
            "read_float", cases=["as_command"], expected_result="11.2345"
        )
        self.check_function(
            "read_boolean", cases=["as_command"], expected_result="True"
        )
    ...
    def test_range(self):
        ...
        self.check_function(
            "range", cases=["as_input"], expected_result="0\n1\n2\n3\n4\n5\n6\n7"
        )

        self.check_function(
            "range", cases=["as_input"], json_file_name="range_0", expected_result=""
        )
    ...
    def test_as_map(self):
        self.check_function(
            "as_map", cases=["as_input"], expected_result='{"a":1,"b":2,"c":3}'
        )
    ...
    def test_collect_by_key(self):
        ...
        self.check_function(
            "collect_by_key", cases=["as_input"], expected_result='{"a":[1,3],"b":[2]}'
        )

    def test_flatten(self):
        self.check_function(
            "flatten", cases=["as_input"], expected_result="[1,2,3,1,21,22]"
        )
