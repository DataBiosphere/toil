...
class ToilWdlIntegrationTest(ToilTest):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.fetch_and_unzip_from_s3(
            filename="ENCODE_data.zip",
            data=cls.encode_data,
            data_dir=cls.encode_data_dir,
        )

        cls.fetch_and_unzip_from_s3(
            filename="wdl_templates.zip", data=cls.wdl_data, data_dir=cls.wdl_data_dir
        )

        cls.fetch_and_unzip_from_s3(
            filename="GATK_data.zip", data=cls.gatk_data, data_dir=cls.gatk_data_dir
        )
    ...
    def testMD5sum(self):
        ...
        subprocess.check_call(
            [exactPython, self.program, wdl, json, "-o", self.output_dir]
        )
    ...
    def testFn_SelectFirst(self):
        ...
        assert (
            select_first(["somestring", "anotherstring", None, "", 1]) == "somestring"
        )
    ...
    def testFn_Size(self) -> None:
        ...
        with Toil(options) as toil:
            small = process_infile(
                WDLFile(
                    file_path=os.path.abspath("src/toil/test/wdl/testfiles/vocab.wdl")
                ),
                toil,
            )
    ...
    def testFn_Basename(self):
        assert (
            basename("/home/quokka/git/delete/toil/src/toil/wdl/toilwdl.py", ".py")
            == "toilwdl"
        )
        assert (
            basename("/home/quokka/git/delete/toil/src/toil/wdl/toilwdl.py")
            == "toilwdl.py"
        )
    ...
    def testFn_Glob(self):
        ...
        assert vocab_location == [
            os.path.abspath("src/toil/test/wdl/testfiles/vocab.wdl")
        ], str(vocab_location)
        wdl_locations = glob("wdl_*.py", os.path.abspath("src/toil"))
        wdl_that_should_exist = [
            os.path.abspath("src/toil/wdl/wdl_analysis.py"),
        ...
        for location in wdl_that_should_exist:
            assert (
                location in wdl_locations
            ), f"{str(location)} not in {str(wdl_locations)}!"
        # make sure the same number of files were found as expected
        assert len(wdl_that_should_exist) == len(
            wdl_locations
        ), f"{str(len(wdl_locations))} != {str(len(wdl_that_should_exist))}"
    ...
    def testFn_ParseDisk(self):
        ...
        assert parse_disk("2200000001") == 2200000001, str(parse_disk("2200000001"))
        assert (
            parse_disk("/mnt/my_mnt 3 SSD, /mnt/my_mnt2 500 HDD") == 503000000000
        ), str(parse_disk("/mnt/my_mnt 3 SSD, /mnt/my_mnt2 500 HDD"))
        assert parse_disk("local-disk 10 SSD") == 10000000000, str(
            parse_disk("local-disk 10 SSD")
        )
        assert parse_disk("/mnt/ 10 HDD") == 10000000000, str(
            parse_disk("/mnt/ 10 HDD")
        )
        assert parse_disk("/mnt/ 1000 HDD") == 1000000000000, str(
            parse_disk("/mnt/ 1000 HDD")
        )
    ...
    def testTut02(self):
        ...
        GATK tutorial #2."""
        wdl = os.path.abspath(
            "src/toil/test/wdl/wdl_templates/t02/simpleVariantSelection.wdl"
        )
        json = os.path.abspath(
            "src/toil/test/wdl/wdl_templates/t02/simpleVariantSelection_inputs.json"
        )
    ...
    def testTut03(self):
        ...
        GATK tutorial #3."""
        wdl = os.path.abspath(
            "src/toil/test/wdl/wdl_templates/t03/simpleVariantDiscovery.wdl"
        )
        json = os.path.abspath(
            "src/toil/test/wdl/wdl_templates/t03/simpleVariantDiscovery_inputs.json"
        )
    ...
    def testTut04(self):
        ...
        GATK tutorial #4."""
        wdl = os.path.abspath(
            "src/toil/test/wdl/wdl_templates/t04/jointCallingGenotypes.wdl"
        )
        json = os.path.abspath(
            "src/toil/test/wdl/wdl_templates/t04/jointCallingGenotypes_inputs.json"
        )
                ...
                for test_file in test_output_files:
                    if file == test_file:
                        ...
                        if file.endswith(".vcf"):
                            compare_vcf_files(
                                filepath1=filepath, filepath2=test_filepath
                            )
                        else:
        ...
        if test_data[i] != good_data[i]:
            for j in range(len(test_data[i])):
                ...
                if j < 5:
                    ...
                    assert test_data[i][j] == good_data[i][j], (
