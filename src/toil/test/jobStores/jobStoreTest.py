...
class AbstractJobStoreTest(object):
    ...
    class Test(with_metaclass(ABCMeta, ToilTest)):
        ...
        #     stats = None
        #     one = b'one' if sys.version_info >= (3, 0) else 'one'
        #     two = b'two' if sys.version_info >= (3, 0) else 'two'
        #
        ...
        def makeImportExportTests(cls):
            # TODO: Make sure this doesn't depend on other hardcoded test classes
            # testClasses = [FileJobStoreTest, AWSJobStoreTest, AzureJobStoreTest, GoogleJobStoreTest]
            testClasses = [AWSJobStoreTest]
            ...
            def testImportExportFile(self, otherCls, size):
                ...
                # with open('/home/quokka/git/toil/src/toil/lib/encryption/log.txt', 'a+') as f:
                #     f.write('store' + str(type(store)) + '\n')
                #     f.write('other' + str(type(other)) + '\n')
                ...
                # with open('/home/quokka/git/toil/src/toil/lib/encryption/log.txt', 'a+') as f:
                #     f.write('srcUrl' + str(type(srcUrl)) + '\n')
                #     f.write('jobStoreFileID' + str(type(jobStoreFileID)) + '\n')
                ...
                # with open('/home/quokka/git/toil/src/toil/lib/encryption/log.txt', 'a+') as f:
                #     f.write('dstUrl' + str(type(dstUrl)) + '\n\n')
...
class AWSJobStoreTest(AbstractJobStoreTest.Test):
    ...
    def _corruptJobStore(self):
        ...
        for key in self.jobstore_initialized.filesBucket.list():
            self.jobstore_initialized.filesBucket.delete_key(key.name)
    ...
    #         overlargeJobNode.jobName = f.read(512 * 1024)
    ...
    def _prepareTestFile(self, bucket, size=None):
        ...
        if size is None:
            return url
        with open('/dev/urandom', 'rb') as f:
            bucket.new_key(fileName).set_contents_from_string(f.read(size))
