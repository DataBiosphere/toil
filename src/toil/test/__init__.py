...
def needs_tes(test_item: MT) -> MT:
    ...
    tes_url = os.environ.get(
        "TOIL_TES_ENDPOINT", TESBatchSystem.get_default_tes_endpoint()
    )
    try:
        urlopen(tes_url)
    except HTTPError:
        ...
        return unittest.skip(f"Run a TES server on {tes_url} to include this test")(
            test_item
        )
    return test_item
