...
class AzureJobStore(AbstractJobStore):
    ...
    def exists(self, jobStoreID):
        if self.jobItems.get_entity(row_key=jobStoreID.encode('utf-8')) is None:
            return False
        return True

    def load(self, jobStoreID):
        jobEntity = self.jobItems.get_entity(row_key=jobStoreID.encode('utf-8'))
    ...
    def delete(self, jobStoreID):
        try:
            self.jobItems.delete_entity(row_key=jobStoreID.encode('utf-8'))
    ...
    def deleteFile(self, jobStoreFileID):
        try:
            self.files.delete_blob(blob_name=jobStoreFileID.encode('utf-8'))
    ...
    def fileExists(self, jobStoreFileID):
        ...
        try:
            self.files.get_blob_metadata(blob_name=jobStoreFileID.encode('utf-8'))
            return True
        except AzureMissingResourceHttpError:
            return False
    ...
    def writeStatsAndLogging(self, statsAndLoggingString):
        ...
        self.statsFiles.create_blob_from_text(blob_name=jobStoreFileID.encode('utf-8'),
            ...
            with attempt:
                for entity in self.statsFileIDs.query_entities():
                    ...
                    if not hasBeenRead:
                        ...
                        self.statsFileIDs.delete_entity(row_key=jobStoreFileID.encode('utf-8'))
                        numStatsFiles += 1
                    elif readAll:
    ...
    def getPublicUrl(self, jobStoreFileID):
        try:
            self.files.get_blob_properties(blob_name=jobStoreFileID.encode('utf-8'))
        except AzureMissingResourceHttpError:
            raise NoSuchFileException(jobStoreFileID)
        startTime = (datetime.utcnow() - timedelta(minutes=5))
        endTime = datetime.utcnow() + self.publicUrlExpiration
        sas_token = self.files.generate_blob_shared_access_signature(blob_name=jobStoreFileID.encode('utf-8'),
                                                                     permission=BlobPermissions.READ,
                                                                     start=startTime,
                                                                     expiry=endTime)
        return self.files.make_blob_url(blob_name=jobStoreFileID.encode('utf-8')) + '?' + sas_token
    ...
    def _dissociateFileFromJob(self, jobStoreFileID):
        ...
        if entities:
            ...
            self.jobFileIDs.delete_entity(partition_key=jobStoreID.encode('utf-8'),
                                          row_key=jobStoreFileID.encode('utf-8'))
    ...
    def _uploadStream(self, jobStoreFileID, container, checkForModification=False, encrypted=None):
        ...
        if checkForModification:
            try:
                expectedVersion = container.get_blob_properties(blob_name=jobStoreFileID.encode('utf-8')).properties.etag
            except AzureMissingResourceHttpError:
            ...
            def readFrom(self, readable):
                ...
                try:
                    while True:
                        ...
                        container.put_block(blob_name=jobStoreFileID.encode('utf-8'),
                                            block=buf,
                                            block_id=blockID)
                    ...
                    with panic(log=logger):
                        ...
                        container.delete_blob(blob_name=jobStoreFileID.encode('utf-8'))

                if checkForModification and expectedVersion is not None:
                    # Acquire a (60-second) write lock,
                    leaseID = container.acquire_blob_lease(blob_name=jobStoreFileID.encode('utf-8'),
                                                           lease_duration=60)
                    # check for modification,
                    blob = container.get_blob_properties(blob_name=jobStoreFileID.encode('utf-8'))
                    if blob.properties.etag != expectedVersion:
                        container.release_blob_lease(blob_name=jobStoreFileID.encode('utf-8'), lease_id=leaseID)
                        raise ConcurrentFileModificationException(jobStoreFileID)
                    # commit the file,
                    container.put_block_list(blob_name=jobStoreFileID.encode('utf-8'),
                                             block_list=blocks,
                                             lease_id=leaseID,
                                             metadata=dict(encrypted=str(encrypted)))
        ...
        class DownloadPipe(ReadablePipe):
            def writeTo(self, writable):
                ...
                while chunkStart < fileSize:
                    ...
                    buf = container.get_blob_to_bytes(blob_name=jobStoreFileID.encode('utf-8'),
...
class AzureJob(JobGraph):
    ...
    def toEntity(self, chunkSize=maxAzureTablePropertySize):
        ...
        item['RowKey'] = self.jobStoreID.encode('utf-8')
        item['PartitionKey'] = AzureTable.defaultPartition.encode('utf-8')
        return Entity(item)
