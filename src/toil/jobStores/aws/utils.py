...
class SDBHelper:
    ...
    def attributesToBinary(cls, attributes):
        ...
        """
        chunks = [
            (int(k), v) for k, v in attributes.items() if cls._isValidChunkName(k)
        ]
        chunks.sort()
...
@retry(
    errors=[ErrorCondition(error=ClientError, error_codes=[404, 500, 502, 503, 504])]
)
def uploadFromPath(
    localFilePath: str,
    resource,
