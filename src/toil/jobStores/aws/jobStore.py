...
class AWSJobStore(AbstractJobStore):
    ...
    def update(self, job):
        ...
        for attempt in retry_sdb():
            with attempt:
                assert self.jobsDomain.put_attributes(str(job.jobStoreID), item)
    ...
    def delete(self, jobStoreID):
        ...
        for attempt in retry_sdb():
            with attempt:
                self.jobsDomain.delete_attributes(item_name=str(jobStoreID))
        items = None
        for attempt in retry_sdb():
            with attempt:
        ...
        if items:
            ...
            for item in items:
                ...
                for attempt in retry_s3():
                    with attempt:
                        if version:
                            self.filesBucket.delete_key(key_name=str(item.name), version_id=version)
                        else:
                            self.filesBucket.delete_key(key_name=str(item.name))
    ...
    def getPublicUrl(self, jobStoreFileID):
        ...
        for attempt in retry_s3():
            with attempt:
                key = self.filesBucket.get_key(key_name=str(jobStoreFileID), version_id=info.version)
    ...
    class FileInfo(SDBHelper):
        ...
        def save(self):
            ...
            try:
                ...
                if self.previousVersion and (self.previousVersion != self.version):
                    for attempt in retry_s3():
                        with attempt:
                            self.outer.filesBucket.delete_key(str(self.fileID),
                ...
                if numNewContentChunks < self._numContentChunks:
                    ...
                    for attempt in retry_sdb():
                        with attempt:
                            self.outer.filesDomain.delete_attributes(str(self.fileID),
                                                                     attributes=attributes)
        ...
        def upload(self, localFilePath):
            ...
            if file_size <= self.maxInlinedSize():
                ...
                self.version = uploadFromPath(localFilePath, partSize=self.outer.partSize,
                                              bucket=self.outer.filesBucket, fileID=str(self.fileID),
                                              headers=headers)
            ...
            class SinglePartPipe(WritablePipe):
                def readFrom(self, readable):
                    ...
                    if allowInlining and len(buf) <= info.maxInlinedSize():
                        ...
                        key = store.filesBucket.new_key(key_name=str(info.fileID))
                        buf = StringIO(buf)
                        headers = info._s3EncryptionHeaders()
                        for attempt in retry_s3():
                            with attempt:
        ...
        def copyTo(self, dstKey):
            ...
            if self.content is not None:
                ...
                for attempt in retry_s3():
                    ...
                    if encrypted:
                        srcKey = self.outer.filesBucket.get_key(str(self.fileID), headers=self._s3EncryptionHeaders())
                    else:
                        srcKey = self.outer.filesBucket.get_key(str(self.fileID))
        ...
        def download(self, localFilePath):
            if self.content is not None:
                ...
                headers = self._s3EncryptionHeaders()
                key = self.outer.filesBucket.get_key(str(self.fileID), validate=False)
                for attempt in retry_s3():
                    with attempt:
        ...
        def delete(self):
            ...
            if self.previousVersion is not None:
                ...
                if self.previousVersion:
                    for attempt in retry_s3():
                        with attempt:
                            store.filesBucket.delete_key(key_name=str(self.fileID),
                                                         version_id=self.previousVersion)
