...
class AWSJobStore(AbstractJobStore):
        ...
        for attempt in retry_sdb():
            with attempt:
                return bool(self.jobsDomain.get_attributes(
                    item_name=compat_str(jobStoreID),
    ...
    def load(self, jobStoreID):
        ...
        for attempt in retry_sdb():
            with attempt:
                item = self.jobsDomain.get_attributes(compat_str(jobStoreID), consistent_read=True)
        if not item:
    ...
    def update(self, job):
        ...
        for attempt in retry_sdb():
            with attempt:
                assert self.jobsDomain.put_attributes(compat_str(job.jobStoreID), item)
    ...
    def delete(self, jobStoreID):
        ...
        for attempt in retry_sdb():
            with attempt:
                self.jobsDomain.delete_attributes(item_name=compat_str(jobStoreID))
        items = None
        for attempt in retry_sdb():
            with attempt:
        ...
        if items:
            ...
            for item in items:
                ...
                for attempt in retry_s3():
                    with attempt:
                        if version:
                            self.filesBucket.delete_key(key_name=compat_str(item.name), version_id=version)
                        else:
                            self.filesBucket.delete_key(key_name=compat_str(item.name))
    ...
    def _getKeyForUrl(url, existing=None):
        ...
        :rtype: Key
        """
        keyName = compat_str(url.path[1:])
        bucketName = compat_str(url.netloc)
    ...
    def getPublicUrl(self, jobStoreFileID):
        ...
        for attempt in retry_s3():
            with attempt:
                key = self.filesBucket.get_key(key_name=compat_str(jobStoreFileID), version_id=info.version)
    ...
    class FileInfo(SDBHelper):
        ...
        def create(cls, ownerID):
            return cls(compat_str(str(uuid.uuid4())), ownerID, encrypted=cls.outer.sseKeyPath is not None)
            ...
            for attempt in retry_sdb():
                with attempt:
                    return bool(cls.outer.filesDomain.get_attributes(
                        item_name=compat_str(jobStoreFileID),
        ...
        def load(cls, jobStoreFileID):
            for attempt in retry_sdb():
                with attempt:
                    self = cls.fromItem(
                        cls.outer.filesDomain.get_attributes(item_name=compat_str(jobStoreFileID),
                                                             consistent_read=True))
                    return self
        ...
        def save(self):
            ...
            try:
                for attempt in retry_sdb():
                    with attempt:
                        assert self.outer.filesDomain.put_attributes(item_name=compat_str(self.fileID),
                                                                     attributes=attributes,
                ...
                if self.previousVersion and (self.previousVersion != self.version):
                    for attempt in retry_s3():
                        with attempt:
                            self.outer.filesBucket.delete_key(compat_str(self.fileID),
                ...
                if numNewContentChunks < self._numContentChunks:
                    ...
                    for attempt in retry_sdb():
                        with attempt:
                            self.outer.filesDomain.delete_attributes(compat_str(self.fileID),
                                                                     attributes=attributes)
        ...
        def upload(self, localFilePath):
            ...
            if file_size <= self.maxInlinedSize():
                ...
                self.version = uploadFromPath(localFilePath, partSize=self.outer.partSize,
                                              bucket=self.outer.filesBucket, fileID=compat_str(self.fileID),
                                              headers=headers)
                        ...
                        for attempt in retry_s3():
                            with attempt:
                                upload = store.filesBucket.initiate_multipart_upload(
                                    key_name=compat_str(info.fileID),
                                    headers=headers)
                        try:
            ...
            class SinglePartPipe(WritablePipe):
                def readFrom(self, readable):
                    ...
                    if allowInlining and len(buf) <= info.maxInlinedSize():
                        ...
                        key = store.filesBucket.new_key(key_name=compat_str(info.fileID))
                        buf = StringIO(buf)
                        headers = info._s3EncryptionHeaders()
                        for attempt in retry_s3():
                            with attempt:
        ...
        def copyFrom(self, srcKey):
            ...
            if srcKey.size <= self.maxInlinedSize():
                ...
                self.version = copyKeyMultipart(srcBucketName=compat_str(srcKey.bucket.name),
                                                srcKeyName=compat_str(srcKey.name),
                                                srcKeyVersion=compat_str(srcKey.version_id),
                                                dstBucketName=compat_str(self.outer.filesBucket.name),
                                                dstKeyName=compat_str(self._fileID),
                                                sseAlgorithm='AES256',
        ...
        def copyTo(self, dstKey):
            ...
            if self.content is not None:
                ...
                for attempt in retry_s3():
                    ...
                    if encrypted:
                        srcKey = self.outer.filesBucket.get_key(compat_str(self.fileID), headers=self._s3EncryptionHeaders())
                    else:
                        srcKey = self.outer.filesBucket.get_key(compat_str(self.fileID))
                    srcKey.version_id = self.version
                    with attempt:
                        copyKeyMultipart(srcBucketName=compat_str(srcKey.bucket.name),
                                         srcKeyName=compat_str(srcKey.name),
                                         srcKeyVersion=compat_str(srcKey.version_id),
                                         dstBucketName=compat_str(dstKey.bucket.name),
                                         dstKeyName=compat_str(dstKey.name),
        ...
        def download(self, localFilePath):
            if self.content is not None:
                ...
                headers = self._s3EncryptionHeaders()
                key = self.outer.filesBucket.get_key(compat_str(self.fileID), validate=False)
                for attempt in retry_s3():
                    with attempt:
            ...
            class DownloadPipe(ReadablePipe):
                def writeTo(self, writable):
                    if info.content is not None:
                        ...
                        headers = info._s3EncryptionHeaders()
                        key = info.outer.filesBucket.get_key(compat_str(info.fileID), validate=False)
                        for attempt in retry_s3():
                            with attempt:
                ...
                for attempt in retry_sdb():
                    with attempt:
                        store.filesDomain.delete_attributes(
                            compat_str(self.fileID),
                            expected_values=['version', self.previousVersion])
                if self.previousVersion:
                    for attempt in retry_s3():
                        with attempt:
                            store.filesBucket.delete_key(key_name=compat_str(self.fileID),
                                                         version_id=self.previousVersion)
