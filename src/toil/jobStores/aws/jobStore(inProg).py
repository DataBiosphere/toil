...
class AWSJobStore(AbstractJobStore):
    ...
    class FileInfo(SDBHelper):
        ...
        def uploadStream(self, multipart=True, allowInlining=True):
            ...
            s3_bucket = boto3.resource('s3').Bucket(store.namePrefix)
            s3_object = s3_bucket.Object(info.fileID)
            ...
            class MultiPartPipe(WritablePipe):
                def readFrom(self, readable)
                    ...
                    if allowInlining and len(buf) <= info.maxInlinedSize():
                        ...
                        mpu = s3_client.create_multipart_upload(Bucket=store.namePrefix, Key=store.locator, **kwargs)
