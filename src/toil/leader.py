...
from multiprocessing import JoinableQueue as Queue, Event
import cPickle
...
####################################################
##Logic to scale the size of the worker cluster
####################################################
...
class ClusterScaler(object):
    def __init__(self, provisioner, batchSystem, config):
        ...
        if config.maxPreemptableNodes > 0:
            ...
            self.preemptableScaler = Process(target=self.scaler, args=args)
        ...
        if config.maxNonPreemptableNodes > 0:
            ...
            self.nonPreemptableScaler = Process(target=self.scaler, args=args)
    ...
    def scaler(provisioner, batchSystem, 
        ...
        :param toil.provisioners.abstractProvisioner.AbstractProvisioner provisioner: Provisioning instance to scale.
        :param toil.batchSystem.abstractBatchSysten.AbstractBatchSystem batchSystem: Batchsystem to monitor to make scaling decisions.
        ...
        try:
            ...
            while True:
                ...
                if ((jobsStartedPerSecond == 0 and queueSize > 0) or 
                    queueSize/jobsStartedPerSecond > maxTimeToRun) and \
                    totalNodes < maxNodes:
                    # Try creating a node
                    try:
                ...
                elif queueSize/jobsStartedPerSecond < minTimeToRun and \
                    totalNodes > minNodes and batchSystem.getNumberOfEmptyNodes() > 0:
                    # Try removing a node
                    try:
