...
class CachingFileStore(AbstractFileStore):
    ...
    def getFileReaderCount(self, fileID):
        ...
        for row in self.cur.execute(
            "SELECT COUNT(*) FROM refs WHERE file_id = ?", (fileID,)
        ):
            return row[0]
        return 0
    ...
    def _getNewCachingPath(self, fileStoreID):
        ...
        handle, path = tempfile.mkstemp(
            dir=self.localCacheDir, suffix=hasher.hexdigest()
        )
        os.close(handle)
        os.unlink(path)
    ...
    def _executePendingDeletions(cls, coordination_dir, con, cur):
        ...
        for row in cur.execute(
            "SELECT id, path FROM files WHERE owner = ? AND state = ?", (me, "deleting")
        ):
    ...
    def writeGlobalFile(self, localFileName, cleanup=False, executable=False):
        ...
        fileID = self.jobStore.getEmptyFileStoreID(
            ...
            if self.cur.fetchone()[0] > 0:
                ...
                if self.cur.fetchone()[0] > 0:
                    ...
                    self._executePendingDeletions(
                        self.coordination_dir, self.con, self.cur
                    )
    ...
    def startCommit(self, jobState=False):
        ...
        self.commitThread = threading.Thread(
            target=self.startCommitThread, args=(jobState,)
        )
    ...
    def startCommitThread(self, jobState):
        ...
        try:
            ...
            if jobState:
                ...
                logger.debug(
                    "Committing file deletes and job state changes asynchronously"
                )
    ...
    def _removeDeadJobs(cls, coordination_dir, con):
        ...
        workers = []
        for row in cur.execute(
            "SELECT DISTINCT worker FROM jobs WHERE worker IS NOT NULL"
        ):
        ...
        while True:
            ...
            # See if we won the race
            cur.execute(
                "SELECT id, tempdir FROM jobs WHERE id = ? AND worker = ?", (jobID, me)
            )
            row = cur.fetchone()
            if row is None:
