...
class Shape:
    ...
    def __hash__(self) -> int:
        ...
        return hash(
            (self.wallTime, self.memory, self.cores, self.disk, self.preemptable)
        )
...
class AbstractProvisioner(ABC):
    ...
    def getKubernetesValues(self, architecture: str = "amd64"):
        ...
        return dict(
            ...
            CLOUD_PROVIDER_SPEC=("cloud-provider: " + self.getKubernetesCloudProvider())
            if self.getKubernetesCloudProvider()
            else "",
        )
    ...
    def addKubernetesWorker(
        ...
        values["WORKER_LABEL_SPEC"] = (
            'node-labels: "eks.amazonaws.com/capacityType=SPOT"' if preemptable else ""
        )
    ...
    def _getIgnitionUserData(
        self, role, keyPath=None, preemptable=False, architecture="amd64"
    ):
        """
        ...
        if role == "worker" and self._leaderWorkerAuthentication is not None:
            ...
            if self.clusterType == "mesos":
                ...
                self.addKubernetesWorker(
                    config, self._leaderWorkerAuthentication, preemptable=preemptable
                )
