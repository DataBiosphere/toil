...
class NodeReservation:
    ...
    def __str__(self) -> str:
        return (
            ...
            % (
                ...
                self.nReservation.shape.preemptable
                if self.nReservation is not None
                else str(None),
                str(len(self.shapes())),
            )
    ...
    def fits(self, jobShape: Shape) -> bool:
        ...
        return (
            ...
            and (jobShape.preemptable or not self.shape.preemptable)
        )
...
class ClusterScaler:
            ...
            for preemptable in (True, False):
                ...
                for nodeShape, instance_type in self.nodeShapeToType.items():
                    nodes_thisType = leader.provisioner.getProvisionedWorkers(
                        instance_type=instance_type, preemptable=preemptable
                    )
        ...
        for nodeShape in self.nodeShapes:
            ...
            if not nodeShape.preemptable:
                ...
                compensationNodes = self._round(
                    self.preemptableNodeDeficit[instance_type] * compensation
                )
                if compensationNodes > 0:
                    logger.debug(
                        "Adding %d non-preemptable nodes of type %s to compensate for a "
                        "deficit of %d preemptable ones.",
                        compensationNodes,
                        instance_type,
    ...
    def updateClusterSize(
        ...
        for nodeShape, estimatedNodeCount in estimatedNodeCounts.items():
            ...
            newNodeCount = self.setNodeCount(
                instance_type, estimatedNodeCount, preemptable=nodeShape.preemptable
            )
            ...
            with attempt:
                ...
                if delta > 0:
                    ...
                    numNodes = numCurrentNodes + self._addNodes(
                        instance_type, numNodes=delta, preemptable=preemptable
                    )
                elif delta < 0:
                    logger.info(
    ...
    def _addNodes(self, instance_type: str, numNodes: int, preemptable: bool) -> int:
        return self.provisioner.addNodes(
            nodeTypes={instance_type}, numNodes=numNodes, preemptable=preemptable
        )
    ...
    def filter_out_static_nodes(
        self, nodes: Dict["Node", NodeInfo], preemptable: bool = False
        ...
        for node, nodeInfo in nodes.items():
            if node:
                ...
                if node.privateIP in self.getStaticNodes(preemptable):
                    ...
                    logger.debug(
                        f"Found {node.privateIP} in {non}preemptable static nodes"
                    )
                else:
                    logger.debug(
                        f"Did not find {node.privateIP} in {non}preemptable static nodes"
                    )
