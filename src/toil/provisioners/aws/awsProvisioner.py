...
def awsFilterImpairedNodes(nodes, ec2):
    ...
    healthyNodes = [
        node for node in nodes if statusMap.get(node.id, None) != "impaired"
    ]
    impairedNodes = [
        node.id for node in nodes if statusMap.get(node.id, None) == "impaired"
    ]
    logger.warning(
...
class AWSProvisioner(AbstractProvisioner):
    def __init__(
        self, clusterName, clusterType, zone, nodeStorage, nodeStorageOverrides, sseKey
    ):
        self.cloud = "aws"
        self._sseKey = sseKey
        ...
        super().__init__(
            clusterName, clusterType, zone, nodeStorage, nodeStorageOverrides
        )
    ...
    def readClusterSettings(self):
        ...
        instance = ec2.get_all_instances(
            instance_ids=[instanceMetaData["instance-id"]]
        )[0].instances[0]
        ...
        self._tags = {
            k: v
            for k, v in self.getLeader().tags.items()
        ...
        self._leaderSecurityGroupNames = (
            {rawSecurityGroups}
            if not isinstance(rawSecurityGroups, list)
            else set(rawSecurityGroups)
        )
    ...
    @retry(
        errors=[
            ErrorCondition(error=ClientError, error_codes=[404, 500, 502, 503, 504])
        ]
    )
    ...
    def getNodeShape(self, instance_type: str, preemptable=False) -> Shape:
        ...
        if disk == 0:
            ...
            # the root volume
            disk = (
                self._nodeStorageOverrides.get(instance_type, self._nodeStorage)
                * 2**30
            )
    ...
    def destroyCluster(self) -> None:
        ...
        if spotIDs:
            self.aws.boto2(self._region, "ec2").cancel_spot_instance_requests(
                request_ids=spotIDs
            )
            removed = True
        instancesToTerminate = awsFilterImpairedNodes(
            instances, self.aws.boto2(self._region, "ec2")
        )
        ...
        for attempt in old_retry(timeout=300, predicate=expectedShutdownErrors):
            with attempt:
                ...
                for ltID in self._get_launch_template_ids():
                    response = self.aws.client(
                        self._region, "ec2"
                    ).delete_launch_template(LaunchTemplateId=ltID)
                    ...
                    for sg in self.aws.boto2(
                        ...
                        if sg.name == self.clusterName and vpcId and sg.vpc_id == vpcId:
                            try:
                                self.aws.boto2(
                                    self._region, "ec2"
                                ).delete_security_group(group_id=sg.id)
                                removed = True
    ...
    def _recover_node_type_bid(
        self, node_type: Set[str], spot_bid: Optional[float]
    ) -> Optional[float]:
        """
    ...
    def addNodes(self, nodeTypes: Set[str], numNodes, preemptable, spotBid=None) -> int:
        ...
        bdm = self._getBoto2BlockDeviceMapping(type_info, rootVolSize=root_vol_size)
        ...
        userData = self._getIgnitionUserData(
            "worker", keyPath, preemptable, self._architecture
        )
        ...
        for attempt in old_retry(predicate=awsRetryPredicate):
            with attempt:
                ...
                if not preemptable:
                    ...
                    # flatten the list
                    instancesLaunched = [
                        item for sublist in instancesLaunched for item in sublist
                    ]
    ...
    def addManagedNodes(
        self, nodeTypes: Set[str], minNodes, maxNodes, preemptable, spotBid=None
    ) -> None:
        ...
        launch_template_ids = {
            n: self._get_worker_launch_template(n, preemptable=preemptable)
            for n in nodeTypes
        }
    ...
    def getProvisionedWorkers(
        ...
        workerInstances = [
            i for i in entireCluster if i.private_ip_address != self._leaderPrivateIP
        ]
        ...
        if preemptable is not None:
            workerInstances = [
                ...
                if preemptable == (i.spot_instance_request_id is not None)
            ]
            logger.debug(
                "%spreemptable workers found in cluster: %s",
                "non-" if not preemptable else "",
                workerInstances,
            )
        workerInstances = awsFilterImpairedNodes(
            workerInstances, self.aws.boto2(self._region, "ec2")
        )
        return [
            Node(
                publicIP=i.ip_address,
                ...
                launchTime=i.launch_time,
                nodeType=i.instance_type,
                preemptable=i.spot_instance_request_id is not None,
                tags=i.tags,
            )
            for i in workerInstances
        ]
    ...
    def _terminateIDs(self, instanceIDs: List[str]):
        ...
        self.aws.boto2(self._region, "ec2").terminate_instances(
    ...
    def _deleteRoles(self, names: List[str]):
        ...
        for role_name in names:
            ...
            for policy_name in self._getRoleInlinePolicyNames(role_name):
                for attempt in old_retry(timeout=300, predicate=expectedShutdownErrors):
                    with attempt:
                        self.aws.client(self._region, "iam").delete_role_policy(
    ...
    def _deleteInstanceProfiles(self, names: List[str]):
        ...
        for profile_name in names:
            for attempt in old_retry(timeout=300, predicate=expectedShutdownErrors):
                with attempt:
                    self.aws.client(self._region, "iam").delete_instance_profile(
    ...
    @classmethod
    def _getBoto2BlockDeviceMapping(
        cls, type_info: InstanceType, rootVolSize: int = 50
    ) -> Boto2BlockDeviceMapping:
        ...
        for disk in range(1, int(type_info.disks) + 1):
            bdm[bdtKeys[disk]] = Boto2BlockDeviceType(
                ephemeral_name=f"ephemeral{disk - 1}"
            )  # ephemeral counts start at 0
    ...
    @awsRetry
    def _get_nodes_in_cluster(
        self, instance_type: Optional[str] = None, include_stopped_nodes=False
    ) -> List[Boto2Instance]:
        """
        Get Boto2 instance objects for all nodes in the cluster.
        """

        all_instances = self.aws.boto2(self._region, "ec2").get_only_instances(
            filters={"instance.group-name": self.clusterName}
        )
    ...
    def _filter_nodes_in_cluster(
        self, instance_type: Optional[str] = None, preemptable: bool = False
    ) -> List[Boto2Instance]:
        """
        Get Boto2 instance objects for the nodes in the cluster filtered by preemptability.
        """

        instances = self._get_nodes_in_cluster(
            instance_type, include_stopped_nodes=False
        )
    ...
    def _getSpotRequestIDs(self) -> List[str]:
        ...
        tags = ec2.get_all_tags(
            {"tag:": {_TAG_KEY_TOIL_CLUSTER_NAME: self.clusterName}}
        )
    ...
    def _createSecurityGroups(self) -> List[str]:
        ...
        try:
            web = vpc.create_security_group(
                self.clusterName, "Toil appliance security group", vpc_id=vpcId
            )
            ...
            for attempt in old_retry(predicate=groupNotFound, timeout=300):
                with attempt:
                    ...
                    web.authorize(
                        ip_protocol="tcp", from_port=0, to_port=65535, src_group=web
                    )
            for attempt in old_retry(predicate=groupNotFound, timeout=300):
                with attempt:
                    # We also want to open up UDP, both for user code and for the RealtimeLogger
                    web.authorize(
                        ip_protocol="udp", from_port=0, to_port=65535, src_group=web
                    )
        out = []
    ...
    def _getSecurityGroupIDs(self) -> List[str]:
        ...
        sgs = [
            ...
            if (
                sg.name in self._leaderSecurityGroupNames
                or sg.id in self._leaderSecurityGroupIDs
            )
        ]
    ...
    def _get_launch_template_ids(
        ...
        while True:
            ...
            if "NextToken" in response:
                ...
                response = ec2.describe_launch_templates(
                    Filters=filters, NextToken=response["NextToken"], MaxResults=200
                )
            else:
                # No more pages
                break
    ...
    def _get_worker_launch_template(
        self, instance_type: str, preemptable: bool = False, backoff: float = 1.0
    ) -> str:
        """
        ...
        lt_name = self._name_worker_launch_template(
            instance_type, preemptable=preemptable
        )
            ...
            try:
                ...
                if (
                    ...
                    return self._get_worker_launch_template(
                        instance_type, preemptable=preemptable, backoff=backoff * 2
                    )
                else:
                    raise
        else:
    ...
    def _name_worker_launch_template(
        self, instance_type: str, preemptable: bool = False
    ) -> str:
        """
    ...
    def _boto2_pager(
        self, requestor_callable: Callable, result_attribute_name: str
    ...
    def _pager(
        self, requestor_callable: Callable, result_attribute_name: str, **kwargs
    ) -> Iterable[Dict[str, Any]]:
        """
    ...
    def _getRoleNames(self) -> List[str]:
        ...
        results = []
        for result in self._boto2_pager(
            self.aws.boto2(self._region, "iam").list_roles, "roles"
        ):
    ...
    def _getInstanceProfileNames(self) -> List[str]:
        ...
        results = []
        for result in self._boto2_pager(
            self.aws.boto2(self._region, "iam").list_instance_profiles,
            "instance_profiles",
        ):
            # For each Boto2 role object
            # Grab out the name
    ...
    def _getRoleInlinePolicyNames(self, role_name: str) -> List[str]:
        ...
        return list(
            self._pager(iam.list_role_policies, "PolicyNames", RoleName=role_name)
        )
    ...
    def _createProfileArn(self) -> str:
        ...
        try:
            ...
            logger.debug(
                "Have preexisting instance profile: %s",
                profile.get_instance_profile_response.get_instance_profile_result.instance_profile,
            )
        except BotoServerError as e:
            if e.status == 404:
                profile = iam.create_instance_profile(iamRoleName)
                profile = (
                    profile.create_instance_profile_response.create_instance_profile_result
                )
                logger.debug(
                    "Created new instance profile: %s", profile.instance_profile
                )
            else:
                raise
        else:
        ...
        for attempt in old_retry(predicate=lambda err: err.status == 404):
            with attempt:
                ...
                iam.add_role_to_instance_profile(
                    profile.instance_profile_name, iamRoleName
                )
