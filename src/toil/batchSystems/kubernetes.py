...
class KubernetesBatchSystem(BatchSystemCleanupSupport):
    ...
    @overload
    def _api(self, kind: Literal["namespace"], max_age_seconds: float = 5 * 60) -> str:
        ...
            ...
            for event in self._stream_until_error(
                ...
                if active_pods > 0:
                    ...
                    if (exitReason == BatchJobExitReason.FAILED) or (
                        ...
                        logger.debug(
                            "Deleting Kubernetes job %s", jobObject.metadata.name
                        )
    ...
    def _getUpdatedBatchJobImmediately(self) -> Optional[UpdatedBatchJobInfo]:
        ...
        try:
            ...
            self._api("batch", errors=[404]).delete_namespaced_job(
                jobObject.metadata.name, self.namespace, propagation_policy="Foreground"
            )
    ...
    def shutdown(self) -> None:
        ...
        try:
            ...
            for jobID in self._getIssuedNonLocalBatchJobIDs():
                ...
                self._api("batch", errors=[]).delete_namespaced_job(
                    jobName, self.namespace, propagation_policy="Background"
                )
    ...
    def killBatchJobs(self, jobIDs: List[int]) -> None:
        ...
        for jobID in jobIDs:
            ...
            response = self._api("batch", errors=[]).delete_namespaced_job(
                jobName, self.namespace, propagation_policy="Foreground"
            )
    ...
    def add_options(cls, parser: Union[ArgumentParser, _ArgumentGroup]) -> None:
        ...
        parser.add_argument(
            ...
            default=cls.get_default_kubernetes_owner(),
            help="Username to mark Kubernetes jobs with.  " "(default: %(default)s)",
        )
        parser.add_argument(
    ...
    def setOptions(cls, setOption: OptionSetter) -> None:
        setOption(
            "kubernetes_host_path", default=None, env=["TOIL_KUBERNETES_HOST_PATH"]
        )
        setOption(
            "kubernetes_owner",
            default=cls.get_default_kubernetes_owner(),
            env=["TOIL_KUBERNETES_OWNER"],
        )
        setOption(
            "kubernetes_service_account",
            default=None,
            env=["TOIL_KUBERNETES_SERVICE_ACCOUNT"],
        )
        setOption(
            "kubernetes_pod_timeout", default=120, env=["TOIL_KUBERNETES_POD_TIMEOUT"]
        )
