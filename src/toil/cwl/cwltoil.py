...
class ToilFsAccessFileStore:
    ...
    def __init__(self, jobstore_uri, localTempDir=None):
        ...
        self.localTempDir = (
            localTempDir or f"/tmp/{uuid.uuid4()}"
...
def main(args: Union[List[str]] = None, stdout: TextIO = sys.stdout) -> int:
    ...
    with Toil(options) as toil:
        if options.restart:
            ...
            picklable_file_store = ToilFsAccessFileStore(
                jobstore_uri=options.jobStore, localTempDir=options.basedir
            )
            runtime_context.make_fs_access = functools.partial(
                ToilFsAccess, picklable_file_store
            )
