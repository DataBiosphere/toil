        ...
        if resourceReq:
            for req in (
                ...
                if isinstance(r, str) and ("$(" in r or "${" in r):
                    ...
                    return job, job
        job = CWLJob(tool, jobobj, runtime_context, container_engine, conditional)  # type: ignore
        return job, job
...
def main(args: Union[List[str]] = None, stdout: TextIO = sys.stdout) -> int:
    ...
    with Toil(options) as toil:
        if options.restart:
            ...
            def set_secondary(fileobj, inp_obj, schema):
                if isinstance(fileobj, Mapping):
                    if fileobj.get("class") == "File":
                        if "secondaryFiles" not in fileobj:
                            ...
                            fileobj["secondaryFiles"] = CommentedSeq()
                            summary: Dict[str, Any] = {}
                            for sf in aslist(inp_obj["secondaryFiles"]):
                                if "$(" in sf["pattern"] or "${" in sf["pattern"]:
                                    result = cwltool.expression.do_eval(
                                        sf["pattern"],
                                        {},
                                        tool.requirements,
                                        outdir,
                                        ...
                                        cwlVersion=loading_context.metadata.get(
                                            "cwlVersion"
                                        ),
                                    ...
                                    for entry in aslist(result):
                                        if entry == None:
                                            continue
                                        elif isinstance(entry, str):
                                            location = urllib.parse.urljoin(
                                                fileobj["location"], entry
                                            )

                                            new_entry = cmap(
                                                {"class": "File", "location": location}
                                            )
                                        else:
                                        ...
                                        for index, existing in enumerate(
                                            ...
                                            if (
                                                ...
                                                if location in summary:
                                                    schema["secondaryFiles"].remove(
                                                        summary[location]
                                                    )
                                                    schema["secondaryFiles"].remove(sf)
                                                done = True
                                        if not done:
                                            fileobj["secondaryFiles"].append(entry)
                                            summary[location] = sf
                                else:
                                    location = cwltool.builder.substitute(
                                        fileobj["location"], sf["pattern"]
                                    )
                                    fileobj["secondaryFiles"].append(
                                        cmap(
                                            {
                                                "location": location,
                                                "class": "File",
                    ...
                    for entry in fileobj:
                        set_secondary(entry, inp_obj)

            def process_job_input(
                inp: Any,
                job_order: Optional[Dict[str, Any]],
                schema: Any,
                skipable: bool = False,
            ) -> None:
                """Set 'streamable' and process secondaryFiles as needed."""
                if job_order == None:
                    return
                if inp["type"] == "File":
                    if skipable and not (
                        "class" in job_order and job_order["class"] == "File"
                    ):
                        return
                    job_order["streamable"] = inp.get("streamable", False)
                    if inp.get("secondaryFiles"):
                        set_secondary(job_order, inp, schema)
                elif isinstance(inp["type"], MutableSequence):
                    for index, entry in enumerate(inp["type"]):
                        new_inp = copy.deepcopy(inp)
                        new_inp["type"] = entry
                        process_job_input(new_inp, job_order, schema, True)
                elif isinstance(inp["type"], Mapping):
                    if "type" in inp["type"]:
                        if inp["type"]["type"] == "record":
                            for field in inp["type"]["fields"]:
                                field_name = shortname(field["name"])
                                if field_name in job_order:
                                    field_job_order = job_order[field_name]
                                    process_job_input(field, field_job_order, schema)
                        elif inp["type"]["type"] == "array":
                            for item in aslist(inp["type"]["items"]):
                                if (
                                    isinstance(item, str)
                                    and item in document_loader.idx
                                ):
                                    item = document_loader.idx[item]
                                for job_order_entry in job_order:
                                    if isinstance(item, str):
                                        if item == "File":
                                            if skipable and not (
                                                isinstance(job_order_entry, Mapping)
                                                and "class" in job_order_entry
                                                and job_order_entry["class"] == "File"
                                            ):
                                                continue
                                            job_order_entry["streamable"] = inp.get(
                                                "streamable", False
                                            )
                                            if inp.get("secondaryFiles"):
                                                set_secondary(
                                                    job_order_entry, inp, schema
                                                )
                                        elif item in (
                                            "boolean",
                                            "int",
                                        ...
                                        if "type" in item:
                                            schema_array_item = schema
                                            if isinstance(schema, Mapping):
                                                for schema_item in aslist(
                                                    schema["type"]["items"]
                                                ):
                                                    if isinstance(schema_item, str):
                                                        ...
                                                        isinstance(schema_item, Mapping)
                                                        and schema_item["type"]
                                                        == "record"
                                                    ):
                                                        for field in schema_item[
                                                            ...
                                                            if field[
                                                                ...
                                                                break
                                                    elif schema_item == item:
                                                        schema_array_item = schema_item
                                                    else:
                                                        raise Exception(
                                                            f"{schema_item}"
                                                        )
                                            ...
                                            raise Exception(
                                                f"Unhandled code path: surprise array item mapping {item} from {inp}"
                                            )
                                    else:
                                        raise Exception(
                                            f"Unhandled code path: surprise array item {item} of {inp}"
                                        )
                        elif inp["type"]["type"] == "enum":
                            return
                        else:
                            raise Exception(
                                f"Unhandled code path: suprise type {inp['type']} of {inp}"
                            )
                    else:
                        raise Exception(f"Unhandled code path: mapping {inp}")
                elif isinstance(inp["type"], str):
                    if inp["type"] in document_loader.idx:
                        resolved_type = copy.deepcopy(inp)
                        resolved_type["type"] = document_loader.idx[inp["type"]]
                        return process_job_input(resolved_type, job_order, schema)
                    elif inp["type"] in (
                        "boolean",
                        "int",
                        "float",
                        ...
                        return
                    raise Exception(f"Unhandled code path: {inp} of type {type(inp)}")
                else:
                    raise Exception(f"Unhandled code path: {inp} of type {type(inp)}")

            for inp in tool.tool["inputs"]:
                norm_id = shortname(inp["id"])
                if norm_id in initialized_job_order:
                    schema = [
                        x
                        for x in tool.inputs_record_schema["fields"]
                        if x["name"] == norm_id
                    ][0]
                    process_job_input(inp, initialized_job_order[norm_id], schema)
