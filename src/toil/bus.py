...
meant to be both generated by and handled in a single Thread, the Toil leader's
main loop thread. If other components send events, they need to do it when
called into by that thread, not from their own daemon threads. Communication
between processes is allowed using MessageBus.connect_output_file() and
MessageBus.scan_bus_messages().
"""
