...
def establish_boto3_session(region_name: Optional[str] = None) -> Session:
    ...
    return Session(botocore_session=botocore_session, region_name=region_name, profile_name=os.environ.get("TOIL_AWS_PROFILE", "default"))
...
class AWSConnectionManager:
    ...
    def boto2(self, region: str, service_name: str) -> boto.connection.AWSAuthConnection:
        ...
        if not hasattr(storage, 'item'):
            storage.item = getattr(boto, service_name).connect_to_region(region, profile_name=os.environ.get("TOIL_AWS_PROFILE", "default"))
