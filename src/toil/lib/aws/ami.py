...
def get_flatcar_ami(ec2_client: BaseClient, architecture: str = "amd64") -> str:
    ...
    while not ami and try_number < MAX_TRIES:
        try_number += 1
        logger.debug(
            "No AMI found in TOIL_AWS_AMI; checking Flatcar release feed (try %s)",
            try_number,
        )
        ami = official_flatcar_ami_release(
...
def official_flatcar_ami_release(
    ...
    try:
        ...
        # We didn't find it
        logger.warning(
            f"Flatcar release feed at {JSON_FEED_URL} does not have an image for region {region}"
        )
    except KeyError:
        # We didn't see a field we need
        logger.warning(
            f"Flatcar release feed at {JSON_FEED_URL} does not have expected format"
        )
    return None
...
def aws_marketplace_flatcar_ami_search(
    ...
    response: dict = ec2_client.describe_images(  # type: ignore
