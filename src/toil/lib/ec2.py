...
def wait_spot_requests_active(ec2, requests: Iterable[SpotInstanceRequest], timeout: float = None,
...
def create_auto_scaling_group(autoscaling_client: BaseClient,
    ...
    mip = {'LaunchTemplate': {'LaunchTemplateSpecification': get_launch_template_spec(next(iter(instance_types))),
                              'Overrides': [
                                  {'InstanceType': t, 'LaunchTemplateSpecification': get_launch_template_spec(t)} for t
                                  in instance_types]}}
