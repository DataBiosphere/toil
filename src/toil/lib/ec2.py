...
from toil.lib.retry import old_retry, retry
...
def wait_until_instance_profile_arn_exists(instance_profile_arn: Dict):
    # The Arn and Name keys are mutually exclusive; we expect 'Arn' here.
    if 'Arn' in instance_profile_arn:
        instance_profile_name = instance_profile_arn['Arn'].split(':instance-profile/')[-1]
        iam_client.get_instance_profile(InstanceProfileName=instance_profile_name)
    elif 'Name' in instance_profile_arn:
        instance_profile_name = instance_profile_arn['Name']
        iam_client.get_instance_profile(InstanceProfileName=instance_profile_name)


@retry(intervals=[5, 5, 10, 20, 20, 20, 20])
