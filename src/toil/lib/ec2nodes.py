...
def fetchEC2InstanceDict(
    awsBillingJson: Dict[str, Any], region: str
    ...
    for k, v in awsBillingJson["products"].items():
        ...
        if (
            i.get("location") == region
            and i.get("tenancy") == "Shared"
            and i.get("operatingSystem") == "Linux"
            and i.get("operation") == "RunInstances"
        ):

            normal_use = i.get("usagetype").endswith(
                "BoxUsage:" + i["instanceType"]
            )  # not reserved or unused
            if normal_use:
                ...
                architecture = (
                    "arm64" if re.search(r".*\dg.*\..*", i["instanceType"]) else "amd64"
                )
...
def updateStaticEC2Instances() -> None:
    ...
    for regionNickname in EC2Regions:
        currentEC2Dict = fetchEC2InstanceDict(
            awsProductDict, region=EC2Regions[regionNickname]
        )
    ...
    for i in sortedCurrentEC2List:
        z = (
            "    '{name}': InstanceType(name='{name}', cores={cores}, memory={memory}, disks={disks}, disk_capacity={disk_capacity}, architecture='{architecture}'),"
            "\n".format(
                name=i.name,
                cores=i.cores,
                memory=i.memory,
