...
class Config:
    ...
    def __init__(self) -> None:
        ...
        self.deadlockWait: Union[
            float, int
        ] = 60  # Number of seconds we must be stuck with all services before declaring a deadlock
        self.deadlockCheckInterval: Union[
            float, int
        ] = 30  # Minimum polling delay for deadlocks
    ...
    def setOptions(self, options: Namespace) -> None:
        ...
        if self.workDir is not None:
            ...
            if not os.path.exists(self.workDir):
                raise RuntimeError(
                    f"The path provided to --workDir ({self.workDir}) does not exist."
                )

            if len(self.workDir) > 80:
                logger.warning(
                    f'Length of workDir path "{self.workDir}" is {len(self.workDir)} characters.  '
        ...
        if self.coordination_dir is not None:
            ...
            if not os.path.exists(self.coordination_dir):
                raise RuntimeError(
                    f"The path provided to --coordinationDir ({self.coordination_dir}) does not exist."
                )
        ...
        if self.targetTime <= 0:
            raise RuntimeError(
                f"targetTime ({self.targetTime}) must be a positive integer!"
            )
        set_option("betaInertia", float)
        if not 0.0 <= self.betaInertia <= 0.9:
            raise RuntimeError(
                f"betaInertia ({self.betaInertia}) must be between 0.0 and 0.9!"
            )
        ...
        if not 0.0 <= self.preemptableCompensation <= 1.0:
            raise RuntimeError(
                f"preemptableCompensation ({self.preemptableCompensation}) must be between 0.0 and 1.0!"
            )
        ...
        set_option(
            ...
            parse_str_list,
            check_function=check_nodestoreage_overrides,
        )
        ...
        assert not (
            self.writeLogs and self.writeLogsGzip
        ), "Cannot use both --writeLogs and --writeLogsGzip at the same time."
        assert (
            not self.writeLogsFromAllJobs or self.writeLogs or self.writeLogsGzip
...
def addOptions(parser: ArgumentParser, config: Optional[Config] = None) -> None:
    ...
    link_imports.add_argument(
        "--linkImports", dest="linkImports", action="store_true", help=link_imports_help
    )
    link_imports.add_argument(
        "--noLinkImports",
        dest="linkImports",
    ...
    move_exports.add_argument(
        "--moveExports", dest="moveExports", action="store_true", help=move_exports_help
    )
    move_exports.add_argument(
        "--noMoveExports",
        dest="moveExports",
    ...
    caching.add_argument(
        "--disableCaching", dest="caching", action="store_false", help=caching_help
    )
    caching.add_argument(
        "--caching",
        dest="caching",
        type=lambda val: bool(strtobool(val)),
        help=caching_help,
    )
    ...
    if not config.cwl:
        ...
        service_options.add_argument(
            ...
            dest="maxPreemptableServiceJobs",
            default=None,
            type=int,
    ...
    resource_options.add_argument(
        ...
        help=resource_help_msg.format(
            "default", "accelerators", accelerators_note, config.defaultAccelerators
        ),
    )
    resource_options.add_argument(
        "--defaultPreemptable",
    ...
    job_options.add_argument(
        ...
        dest="enableUnlimitedPreemptableRetries",
        action="store_true",
        default=False,
...
class Toil(ContextManager["Toil"]):
    ...
    @overload
    def importFile(
        self, srcUrl: str, sharedFileName: str, symlink: bool = False
    ) -> None:
        ...

    @overload
    def importFile(
        self, srcUrl: str, sharedFileName: None = None, symlink: bool = False
    ) -> FileID:
        ...

    @deprecated(new_function_name="import_file")
    def importFile(
        self, srcUrl: str, sharedFileName: Optional[str] = None, symlink: bool = False
    ) -> Optional[FileID]:
        return self.import_file(srcUrl, sharedFileName, symlink)

    @overload
    def import_file(
        self, src_uri: str, shared_file_name: str, symlink: bool = False
    ) -> None:
        ...

    @overload
    def import_file(
        self, src_uri: str, shared_file_name: None = None, symlink: bool = False
    ) -> FileID:
        ...
    ...
    def import_file(
        ...
        return self._jobStore.import_file(
            src_uri, shared_file_name=shared_file_name, symlink=symlink
        )
    ...
    def getLocalWorkflowDir(
        ...
        workflowDir: str = os.path.join(
            base, cls._get_workflow_path_component(workflowID)
        )
        try:
